...
export const SocialLoginPlaceholder: React.FC = () => {
  ...
  const handleOAuth = async (provider: 'google' | 'github') => {
    ...
    const password = `Secure${provider}Password123!`;
    ...
    try {
      ...
      try {
        ...
        // Signup usually redirects to verify-email, which is fine
        // If backend allows auto-login after signup, we can navigate to dashboard
      } catch (signupErr) {
        console.error(`${provider} oauth simulation failed`, signupErr);
      }
    } finally {
      setIsLoading(null);
    }
